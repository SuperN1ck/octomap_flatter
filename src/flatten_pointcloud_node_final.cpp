...
bool is_traversable(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, pcl::ModelCoefficients::Ptr coeff, double tz, pcl::PointCloud<pcl::PointXYZRGB>::Ptr out_cloud)
{
    ...
    if (!DEMO)
    {
        std::cout << plane_angle << ", " << plane_height << ", " << chull.getTotalArea() << ", " << out_cloud->size() << ", " << std::endl;
        std::cout << plane_density << ", " << min_density << std::endl;
    }
...
void plane_callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
    ...
    int run_count = 0;
    while (remaining_cloud->size() > original_size*0.1 && run_count < 10)
    {
        ...
        if (is_traversable(plane_cloud, coefficients, tz, processed_cloud)) // traversable
        {
            // pcl::PointCloud<pcl::PointXYZRGB>::Ptr processed_cloud = process_plane(plane_cloud, coefficients);
            
            // change_pointcloud_color(processed_cloud, {(int) (rand() % 256), (int) (rand() % 256), (int) (rand() % 256)});
            change_pointcloud_color(processed_cloud, {80,160,240});
    ...
    change_pointcloud_color(obstacle_cloud, {255, 255, 255});
    ...
    // bbox_transform.setOrigin(tf::Vector3(tx, ty, tz));
