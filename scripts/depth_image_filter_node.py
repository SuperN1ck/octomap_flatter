...
class SubThenFilter:
    def __init__(self, sub_topic, pub_topic):
        ...
        self.max_threshold_uint16 = 2000
    ...
    def image_callback(self, data):
        ...
        if self.use_inpaiting:
            ...
            while True and iterations < self.inpaiting_iterations:
                ...
                cv_image = cv2.inpaint(cv_image , mask, 11, cv2.INPAINT_NS)
                iterations += 1
        ...
        if self.use_median_blur:
            ...
            # while True:
            for filter_size in [201, 101, 51, 33, 19, 5, 3]:
                mask = cv_image > max_threshold_uint8
                if not mask.any():
                    break
                indices = np.where(mask)
                cv_image_2 = cv2.medianBlur(cv_image, filter_size)
